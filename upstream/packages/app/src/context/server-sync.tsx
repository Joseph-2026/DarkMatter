...
import { NormalizedProviderListResponse } from "@apt5/session-ui/context"
...
export const loadLspQuery = (scope: ServerScope, directory: string, sdk: Apt5Client) =>
  queryOptions({
...
function makeQueryOptionsApi(
  scope: ServerScope,
  serverSDK: () => Apt5Client,
  serverAPI: ServerApi,
  sdkFor: (dir: PathKey) => Apt5Client,
  protocol: Promise<"v1" | "v2">,
) {
  return {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const sdkCache = new Map<string, Apt5Client>()
