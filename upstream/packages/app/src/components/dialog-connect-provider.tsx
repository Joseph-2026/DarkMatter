import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@apt5/client/promise"
...
import { List, type ListRef } from "@apt5/ui/list"
import { ProviderIcon } from "@apt5/ui/provider-icon"
import { Spinner } from "@apt5/ui/spinner"
import { Tag } from "@apt5/ui/tag"
import { TextField } from "@apt5/ui/text-field"
import { ButtonV2 } from "@apt5/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@apt5/ui/v2/dialog-v2"
import { TextInputV2 } from "@apt5/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "opencode-go") return language.t("dialog.provider.apt5Go.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "darkmatter"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.apt5.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["darkmatter", "opencode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "darkmatter" || provider.id === "opencode-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "darkmatter"
                                ? "dialog.provider.apt5.tagline"
                                : "dialog.provider.apt5Go.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "darkmatter" && platform.platform === "desktop") {
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "darkmatter"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.apt5Zen.line1")}</div>
              <div>{language.t("provider.connect.apt5Zen.line2")}</div>
              <div>
                {language.t("provider.connect.apt5Zen.visit.prefix")}
                <ExternalLink
                  href="https://github.com/Joseph-2026/DarkMatter/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.apt5Zen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.apt5Zen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "darkmatter"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.apt5Zen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.apt5Zen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.apt5Zen.visit.prefix")}
                <ExternalLink href="https://github.com/Joseph-2026/DarkMatter/zen" tabIndex={-1}>
                  {language.t("provider.connect.apt5Zen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.apt5Zen.visit.suffix")}
              </div>
