...
const PROVIDER_NOTES = [
  { match: (id: string) => id === "darkmatter", key: "dialog.provider.apt5.note" },
  { match: (id: string) => id === "opencode-go", key: "dialog.provider.apt5Go.tagline" },
...
const SettingsProvidersContent: Component<{ onBack?: () => void }> = (props) => {
  ...
  const connected = createMemo(() => {
    return providers
      .connected()
      .filter((p) => p.id !== "darkmatter" || Object.values(p.models).find((m) => m.cost?.input))
  })
