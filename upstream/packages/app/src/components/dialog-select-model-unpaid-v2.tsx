...
import { Icon } from "@apt5/ui/v2/icon"
...
import { useTheme } from "@apt5/ui/theme"
...
const featuredProviders = ["darkmatter", "opencode-go", "openai", "anthropic", "google", "github-copilot"]
...
export const DialogSelectModelUnpaidV2: Component<{ model?: ModelState }> = (props) => {
  ...
  const isFree = (item: ReturnType<ModelState["list"]>[number]) =>
    item.provider.id === "darkmatter" && (!item.cost || item.cost.input === 0)
