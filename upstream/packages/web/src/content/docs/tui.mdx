---
title: TUI
description: Using the APT-5 terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

APT-5 provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running APT-5 starts the TUI for the current directory.
...
When using the APT-5 TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to APT-5. Allows you to select from available providers and add their API keys.
...
- `leader_timeout` - Controls how long APT-5 waits after the leader key. Defaults to `2000`.
...
Use `APT5_TUI_CONFIG` to load a custom TUI config path.
