...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `APT5_EXPERIMENTAL_LSP_TOOL=true` (lub `APT5_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenCode lub gdy zmienna środowiskowa `APT5_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).
