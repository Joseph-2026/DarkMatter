...
:::note
Ovaj alat je dostupan samo kada je `APT5_EXPERIMENTAL_LSP_TOOL=true` (ili `APT5_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `APT5_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
