...
3. **Prilagođena konfiguracija** (`APT5_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`opencode.json` u projektu) - postavke specifične za projekat
5. **`.apt5` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`APT5_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.apt5` i `~/.config/opencode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `APT5_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `APT5_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.apt5` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.apt5`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`github.com/Joseph-2026/DarkMatter/config.json`**](https://github.com/Joseph-2026/DarkMatter/config.json).

TUI konfiguracija koristi [**`github.com/Joseph-2026/DarkMatter/tui.json`**](https://github.com/Joseph-2026/DarkMatter/tui.json).
...
Koristite `APT5_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/opencode/agents/` ili `.apt5/agents/`. [Saznajte više ovdje](/docs/agents).
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/opencode/commands/` ili `.apt5/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.apt5/plugins/` ili `~/.config/opencode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
