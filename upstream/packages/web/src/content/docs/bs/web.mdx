...
:::caution
Ako `APT5_SERVER_PASSWORD` nije postavljen, server će biti nezaštićen. Ovo je u redu za lokalnu upotrebu, ali bi trebalo biti postavljeno za pristup mreži.
:::
...
Da biste zaštitili pristup, postavite lozinku koristeći varijablu okruženja `APT5_SERVER_PASSWORD`:

```bash
APT5_SERVER_PASSWORD=secret opencode web
```

Korisničko ime podrazumevano je `opencode`, ali se može promeniti sa `APT5_SERVER_USERNAME`.
