---
title: Custom Tools
description: Create tools the LLM can call in apt-5.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside apt-5's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.apt5/tools/` directory of your project.
- Or globally, by placing them in `~/.config/darkmatter/tools/`.
...
```ts title=".apt5/tools/database.ts" {1}
...
```ts title=".apt5/tools/project.ts" {8}
import { tool } from "@apt5/plugin"
...
```python title=".apt5/tools/add.py"
import sys
...
```ts title=".apt5/tools/python-add.ts" {10}
import { tool } from "@apt5/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".apt5/tools/add.py")
