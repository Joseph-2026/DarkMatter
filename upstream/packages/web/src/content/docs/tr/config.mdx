...
Örneğin, genel ayarınız `theme: "darkmatter"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
3. **Özel yapılandırma** (`APT5_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `opencode.json`) - projeye özgü ayarlar
5. **`.apt5` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`APT5_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.apt5` ve `~/.config/opencode` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
`APT5_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`APT5_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.apt5` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.apt5` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.

---

## Şema

Yapılandırma dosyası [**`github.com/Joseph-2026/DarkMatter/config.json`**](https://github.com/Joseph-2026/DarkMatter/config.json)'da tanımlanan bir şemaya sahiptir.
...
Agent'ları `~/.config/opencode/agents/` veya `.apt5/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Komutları `~/.config/opencode/commands/` veya `.apt5/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Eklenti dosyalarını `.apt5/plugins/` veya `~/.config/opencode/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
