...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `APT5_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `opencode` değeridir; değiştirmek isterseniz `APT5_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `opencode serve` hem de `opencode web` için geçerlidir.

```bash
APT5_SERVER_PASSWORD=your-password opencode serve
```
