...
Create `.apt5/commands/test.md`:

```md title=".apt5/commands/test.md"
---
...
You can add custom commands through the APT-5 config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your APT-5 [config](/docs/config):

```json title="apt5.jsonc" {4-12}
{
...
- Global: `~/.config/darkmatter/commands/`
- Per-project: `.apt5/commands/`

```markdown title="~/.config/darkmatter/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".apt5/commands/component.md"
---
...
```md title=".apt5/commands/create-file.md"
---
...
```md title=".apt5/commands/analyze-coverage.md"
---
...
```md title=".apt5/commands/review-changes.md"
---
...
```md title=".apt5/commands/review-component.md"
---
...
apt-5 includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
