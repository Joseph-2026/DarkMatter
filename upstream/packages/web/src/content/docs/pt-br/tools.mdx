...
:::note
Esta ferramenta está disponível apenas quando `APT5_EXPERIMENTAL_LSP_TOOL=true` (ou `APT5_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor opencode ou quando a variável de ambiente `APT5_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).
