...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `APT5_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `APT5_SERVER_USERNAME` ou `opencode`) |
...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencode sem a interface TUI. Defina `APT5_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencode através de uma interface web. Defina `APT5_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `APT5_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `APT5_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `APT5_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `APT5_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `APT5_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `APT5_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `APT5_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `APT5_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `APT5_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `APT5_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `APT5_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `APT5_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `APT5_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `APT5_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `APT5_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `APT5_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `APT5_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `APT5_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `APT5_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `APT5_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `APT5_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `APT5_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opencode`) |
| `APT5_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                        | Tipo    | Descrição                                                 |
| ----------------------------------------------- | ------- | --------------------------------------------------------- |
| `APT5_EXPERIMENTAL`                         | boolean | Habilitar recursos experimentais sob o sinalizador global |
| `APT5_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `APT5_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `APT5_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `APT5_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `APT5_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `APT5_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `APT5_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `APT5_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `APT5_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `APT5_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar TY LSP para arquivos python                     |
| `APT5_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
| `APT5_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Habilitar tarefas de subagentes em segundo plano          |
| `APT5_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Habilitar sistema de eventos experimental                 |
| `APT5_EXPERIMENTAL_NATIVE_LLM`              | boolean | Habilitar caminho nativo de requisição LLM                |
| `APT5_EXPERIMENTAL_PARALLEL`                | boolean | Habilitar execução paralela de busca web                  |
| `APT5_EXPERIMENTAL_SCOUT`                   | boolean | Habilitar subagente Scout                                 |
| `APT5_EXPERIMENTAL_WORKSPACES`              | boolean | Habilitar suporte a espaços de trabalho                   |
