...
3. **Configuração personalizada** (`APT5_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`opencode.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.apt5`** - agentes, comandos, plugins
6. **Configuração inline** (`APT5_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.apt5` e `~/.config/opencode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `APT5_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `APT5_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.apt5`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.apt5`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`github.com/Joseph-2026/DarkMatter/config.json`**](https://github.com/Joseph-2026/DarkMatter/config.json).

A configuração do TUI usa [**`github.com/Joseph-2026/DarkMatter/tui.json`**](https://github.com/Joseph-2026/DarkMatter/tui.json).
...
Use `APT5_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.apt5/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.apt5/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.apt5/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
