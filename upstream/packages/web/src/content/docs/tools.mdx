...
Tools allow the LLM to perform actions in your codebase. APT-5 comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in APT-5.
...
:::note
This tool is only available when `APT5_EXPERIMENTAL_LSP_TOOL=true` (or `APT5_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the APT-5 or the low-cost open-model subscription provider, or when either the `APT5_ENABLE_EXA` or `APT5_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching APT-5:

```bash
APT5_ENABLE_EXA=1 apt-5
# or
APT5_ENABLE_PARALLEL=1 apt-5
```
