...
With APT-5 you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, APT-5 uses our own `opencode` theme.
...
APT-5 comes with several built-in themes.
...
- Want APT-5 to match their terminal's appearance
...
APT-5 supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/darkmatter/themes/*.json` or `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.apt5/themes/*.json`
4. **Current working directory** - Defined in `./.apt5/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/darkmatter/themes
vim ~/.config/darkmatter/themes/my-theme.json
```
...
{
  "$schema": "https://github.com/Joseph-2026/DarkMatter/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
