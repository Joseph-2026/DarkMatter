...
3. **Tilpasset konfiguration** (`APT5_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`opencode.json` i projekt) - projektspecifikke indstillinger
5. **`.apt5` mapper** - agent, kommandoer, plugins
6. **Inline config** (`APT5_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.apt5` og `~/.config/opencode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `APT5_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `APT5_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.apt5` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.apt5` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`github.com/Joseph-2026/DarkMatter/config.json`**](https://github.com/Joseph-2026/DarkMatter/config.json).
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opencode/agents/` eller `.apt5/agents/`. [Learn more here](/docs/agents).
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opencode/commands/` eller `.apt5/commands/`. [Learn more here](/docs/commands).
...
Placer plugin-filer i `.apt5/plugins/` eller `~/.config/opencode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
