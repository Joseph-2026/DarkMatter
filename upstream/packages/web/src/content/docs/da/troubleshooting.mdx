...
Hvis du har `APT5_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `APT5_PORT` (eller vælg en ledig port) og start på ny.
