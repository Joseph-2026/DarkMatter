...
There are two types of agents in APT-5; primary agents and subagents.
...
APT-5 comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
APT-5 comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

APT-5 comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into APT-5's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
- Global: `~/.config/darkmatter/agents/`
- Per-project: `.apt5/agents/`

```markdown title="~/.config/darkmatter/agents/review.md"
---
...
If no temperature is specified, APT-5 uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global APT-5 config and the project specific config.
...
The model ID in your APT-5 config uses the format `provider/model-id`. For example, if you're using [the curated model gateway](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `apt-5 models` to see a list of the available models.
:::
...
```markdown title="~/.config/darkmatter/agents/docs-writer.md"
---
...
```markdown title="~/.config/darkmatter/agents/security-auditor.md"
---
