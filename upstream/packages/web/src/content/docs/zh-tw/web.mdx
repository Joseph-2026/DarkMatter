...
:::caution
如果未設定 `APT5_SERVER_PASSWORD`，伺服器將沒有安全保護。本地使用沒有問題，但在網路存取時應當設定密碼。
:::
...
要保護伺服器存取，可以透過 `APT5_SERVER_PASSWORD` 環境變數設定密碼：

```bash
APT5_SERVER_PASSWORD=secret opencode web
```

使用者名稱預設為 `opencode`，可以透過 `APT5_SERVER_USERNAME` 進行更改。
