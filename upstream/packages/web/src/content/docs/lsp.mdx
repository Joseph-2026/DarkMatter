---
title: LSP Servers
description: APT-5 integrates with your LSP servers.
---

APT-5 can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

APT-5 comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `APT5_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and apt-5 opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your apt-5 config.
