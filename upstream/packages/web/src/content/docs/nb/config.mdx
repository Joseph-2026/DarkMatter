...
For eksempel, hvis din globale konfigurasjon setter `theme: "darkmatter"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
3. **Egendefinert konfigurasjon** (`APT5_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`opencode.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.apt5` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`APT5_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.apt5` og `~/.config/opencode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `APT5_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `APT5_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.apt5` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.apt5`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`github.com/Joseph-2026/DarkMatter/config.json`**](https://github.com/Joseph-2026/DarkMatter/config.json).
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/opencode/agents/` eller `.apt5/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/opencode/commands/` eller `.apt5/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.apt5/plugins/` eller `~/.config/opencode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
