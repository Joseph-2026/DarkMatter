...
To debug issues with APT-5, start by checking the logs and local data it stores on disk.
...
- **macOS/Linux**: `~/.local/share/darkmatter/log/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\apt-5\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `apt-5 --log-level DEBUG`.

---

## Storage

apt-5 stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/darkmatter/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\apt-5`
...
To uninstall the APT-5 CLI and remove its related files, run:
...
To remove APT-5 Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

APT-5 Desktop runs a local APT-5 server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `APT-5` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/darkmatter/opencode.jsonc` (or `~/.config/darkmatter/opencode.json`)
- **macOS/Linux** (older installs): `~/.local/share/darkmatter/opencode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\apt-5\opencode.jsonc`
...
APT-5 can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/darkmatter/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\apt-5\plugins`
- **Project plugins** (only if you use per-project config)
  - `<your-project>/.apt5/plugins/`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so APT-5 can rebuild it.

1. Quit APT-5 Desktop completely.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\apt-5`

3. Restart APT-5 Desktop.

---

### Fix server connection issues

APT-5 Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If you have `APT5_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `APT5_PORT` (or pick a free port) and restart.
...
On Windows, APT-5 Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with APT-5's features.

---

### Notifications not showing

APT-5 Desktop only shows system notifications when:

- notifications are enabled for APT-5 in your OS settings, and
...
1. Quit APT-5 Desktop.
2. Find and delete these files (they live in the APT-5 Desktop app data directory):
...
If you're experiencing issues with APT-5:
...
2. **Join our Discord**
   ...
   [**github.com/Joseph-2026/DarkMatter/discord**](https://github.com/Joseph-2026/DarkMatter/discord)
...
3. Ensure you have the latest version with `apt-5 upgrade`
...
To figure out what models you have access to, run `apt-5 models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/darkmatter
   ```

   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\apt-5`
...
If you encounter API call errors, this may be due to outdated provider packages. apt-5 dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\apt-5`

2. Restart apt-5 to reinstall the latest provider packages

This will force apt-5 to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
apt-5 will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
