...
APT-5 uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start APT-5 with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
apt-5 --auto
```

You can also use auto mode with [`apt-5 run`](/docs/cli#run).

```bash
apt-5 run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where APT-5 was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
APT-5 permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, APT-5 starts from permissive defaults:
...
When APT-5 prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current APT-5 session)
