...
:::note
هذه الأداة متاحة فقط عند ضبط `APT5_EXPERIMENTAL_LSP_TOOL=true` (أو `APT5_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `APT5_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
