...
On macOS, OpenCode reads managed preferences from the `ai.apt5.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.
...
Use the `ai.apt5.managed` PayloadType. The OpenCode config keys go directly in the payload dict:
