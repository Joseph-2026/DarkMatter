---
title: Providers
description: Using any LLM provider in APT-5.
---

import config from "../../../config.mjs"
export const console = config.console

APT-5 uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your APT-5 config.
...
in `~/.local/share/darkmatter/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your APT-5
config.
...
## the curated model gateway

The curated model gateway is a list of models provided by the APT-5 team that have been
tested and verified to work well with APT-5. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with the curated model gateway.
:::

1. Run the `/connect` command in the TUI, select `OpenCode Zen`, and head to [github.com/Joseph-2026/DarkMatter/auth](https://github.com/Joseph-2026/DarkMatter/zen).
...
It works like any other provider in APT-5 and is completely optional to use.

---

## the low-cost open-model subscription

The low-cost open-model subscription is a low cost subscription plan that provides reliable access to popular open coding models provided by the APT-5 team that have been
tested and verified to work well with APT-5.

1. Run the `/connect` command in the TUI, select `OpenCode Go`, and head to [github.com/Joseph-2026/DarkMatter/auth](https://github.com/Joseph-2026/DarkMatter/zen).
...
To use Amazon Bedrock with APT-5:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running apt-5:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY apt-5

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile apt-5

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX apt-5
   ```
...
APT-5. Anthropic explicitly prohibits this.

Previous versions of APT-5 came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in APT-5 with zero setup:
...
You can configure apt-5 to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="apt5.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/Joseph-2026/DarkMatter/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for apt-5 to work properly.
   :::
...
APT-5 does not query Azure management APIs or discover deployments. Select a model whose catalog name matches your deployment, or configure its deployment name explicitly:
...
Assign your identity the inference role required by the deployment: **Cognitive Services OpenAI User** for Azure OpenAI models or **Cognitive Services User** for other Foundry models. APT-5 refreshes access tokens through the Azure CLI, including versions earlier than 2.54.0, so you only need to sign in again when the CLI session expires.
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX apt-5
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your apt-5 config.
...
APT-5 supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; APT-5 uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize APT-5. Sign in and approve.

   :::note
   APT-5 requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, APT-5 surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — APT-5 forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in APT-5 is experimental. Features, configuration, and
behavior may change in future releases.
:::

APT-5 integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in apt-5 auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
APT-5 uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock APT-5
...
When a `duo-workflow-*` model is selected, APT-5 will:
...
4. Route tool execution requests through APT-5's permission-gated tool system
...
To use your GitHub Copilot subscription with apt-5:
...
To use Google Vertex AI with APT-5:
...
2. Set the required environment variables:
   ...
   Set them while running apt-5.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id apt-5
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through apt-5, you can always configure it yourself.
...
```jsonc title="~/.config/darkmatter/opencode.jsonc"
{
  "$schema": "https://github.com/Joseph-2026/DarkMatter/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each APT-5 conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each APT-5 conversation listed as a separate session.
...
You can configure apt-5 to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="apt5.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://github.com/Joseph-2026/DarkMatter/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure apt-5 to use local models through LM Studio.

```json title="apt5.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/Joseph-2026/DarkMatter/config.json",
  "provider": {
    "lmstudio": {
...
You can configure apt-5 to use local models through Ollama.

:::tip
Ollama can automatically configure itself for APT-5. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::

```json title="apt5.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/Joseph-2026/DarkMatter/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with APT-5:
...
3. Copy the API key for use in APT-5.
...
6. **Important**: Before using cloud models in APT-5, you must pull the model information locally:
...
The curated model gateway is a list of tested and verified models provided by the APT-5 team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>the curated model gateway</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **the curated model gateway**.

   ```txt
   /connect
   ```

3. Enter your APT-5 API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your apt-5 config.
...
5. You can also customize them through your apt-5 config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' apt-5
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group apt-5
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with APT-5:
...
apt-5's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. APT-5 captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting apt-5:
...
You can also customize models through your apt-5 config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. APT-5 opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by APT-5. APT-5 polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
APT-5 refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with apt-5. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `opencode.json` file in your project directory:
   ...
   ```json title="apt5.json" ""myprovider"" {5-15}
   {
     "$schema": "https://github.com/Joseph-2026/DarkMatter/config.json",
     "provider": {
       "myprovider": {
...
```json title="apt5.json" {9,11,17-20}
{
  "$schema": "https://github.com/Joseph-2026/DarkMatter/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow APT-5 to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `apt-5 auth list` to see if the credentials
...
2. For custom providers, check the apt-5 config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your apt-5 config.
