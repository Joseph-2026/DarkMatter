...
You can add external tools to APT-5 using the _Model Context Protocol_, or MCP. APT-5 supports both local and remote servers.
...
You can define MCP servers in your [APT-5 Config](https://github.com/Joseph-2026/DarkMatter/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="apt5.jsonc" {6}
{
...
APT-5 automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, APT-5 will:
...
If the server requires authentication, APT-5 will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `apt-5 mcp auth <server-name>`.
...
```bash
apt-5 mcp auth my-oauth-server
```
...
```bash
apt-5 mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, APT-5 will store the tokens securely in `~/.local/share/darkmatter/mcp-auth.json`.
...
apt-5 mcp debug my-oauth-server
```
...
Your MCPs are available as tools in APT-5, alongside built-in tools. So you can manage them through the APT-5 config like any other tool.
...
```bash
apt-5 mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect APT-5 to your Sentry account.
