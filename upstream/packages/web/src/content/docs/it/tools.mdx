...
:::note
Questo strumento e' disponibile solo quando `APT5_EXPERIMENTAL_LSP_TOOL=true` (oppure `APT5_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OpenCode o quando la variabile d'ambiente `APT5_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).
