...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `APT5_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `APT5_SERVER_USERNAME` o `opencode`) |
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `APT5_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `APT5_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `APT5_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `APT5_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `APT5_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `APT5_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `APT5_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `APT5_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `APT5_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `APT5_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `APT5_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `APT5_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `APT5_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `APT5_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `APT5_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `APT5_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `APT5_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `APT5_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `APT5_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `APT5_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `APT5_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `APT5_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `APT5_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `APT5_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `APT5_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `APT5_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `APT5_EXPERIMENTAL`                         | boolean | Abilita funzionalità sperimentali sotto il flag globale |
| `APT5_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                                  |
| `APT5_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI                     |
| `APT5_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms               |
| `APT5_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM                |
| `APT5_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir                   |
| `APT5_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                                 |
| `APT5_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale                      |
| `APT5_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                                 |
| `APT5_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali                   |
| `APT5_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `APT5_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `APT5_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `APT5_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `APT5_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `APT5_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `APT5_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `APT5_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |
