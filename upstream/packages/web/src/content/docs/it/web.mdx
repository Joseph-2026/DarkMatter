...
:::caution
Se `APT5_SERVER_PASSWORD` non e impostata, il server non sara protetto. Va bene per l'uso locale, ma va impostata per l'accesso in rete.
:::
...
Per proteggere l'accesso, imposta una password usando la variabile d'ambiente `APT5_SERVER_PASSWORD`:

```bash
APT5_SERVER_PASSWORD=secret opencode web
```

Lo username predefinito e `opencode`, ma puo essere cambiato con `APT5_SERVER_USERNAME`.
