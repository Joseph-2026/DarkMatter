...
3. **Config personalizzata** (variabile d'ambiente `APT5_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`opencode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.apt5`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `APT5_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.apt5` e `~/.config/opencode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `APT5_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `APT5_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.apt5` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.apt5`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`github.com/Joseph-2026/DarkMatter/config.json`**](https://github.com/Joseph-2026/DarkMatter/config.json).

La configurazione TUI usa [**`github.com/Joseph-2026/DarkMatter/tui.json`**](https://github.com/Joseph-2026/DarkMatter/tui.json).
...
Usa `APT5_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.apt5/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.apt5/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.apt5/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
