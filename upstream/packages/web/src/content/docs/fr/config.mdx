...
3. **Configuration personnalisée** (`APT5_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`opencode.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.apt5`** - agents, commandes, plugins
6. **Configuration en ligne** (`APT5_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.apt5` et `~/.config/opencode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `APT5_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `APT5_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.apt5`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.apt5`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`github.com/Joseph-2026/DarkMatter/config.json`**](https://github.com/Joseph-2026/DarkMatter/config.json).

La configuration TUI utilise [**`github.com/Joseph-2026/DarkMatter/tui.json`**](https://github.com/Joseph-2026/DarkMatter/tui.json).
...
Utilisez `APT5_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/opencode/agents/` ou `.apt5/agents/`. [En savoir plus ici](/docs/agents).
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/opencode/commands/` ou `.apt5/commands/`. [En savoir plus ici](/docs/commands).
...
Placez les fichiers du plugin dans `.apt5/plugins/` ou `~/.config/opencode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
