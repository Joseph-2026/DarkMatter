...
:::caution
Si `APT5_SERVER_PASSWORD` n'est pas défini, le serveur ne sera pas sécurisé. C'est bien pour une utilisation locale mais doit être défini pour l'accès au réseau.
:::
...
Pour protéger l'accès, définissez un mot de passe à l'aide de la variable d'environnement `APT5_SERVER_PASSWORD` :

```bash
APT5_SERVER_PASSWORD=secret opencode web
```

Le nom d'utilisateur par défaut est `opencode` mais peut être modifié avec `APT5_SERVER_USERNAME`.
