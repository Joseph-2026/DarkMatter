...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Mot de passe d'authentification de base (par défaut `APT5_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `APT5_SERVER_USERNAME` ou `opencode`) |
...
Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'OpenCode sans l'interface TUI. Définissez `APT5_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à OpenCode via une interface Web. Définissez `APT5_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------------------- |
| `APT5_AUTO_SHARE`                 | booléen | Partager automatiquement des sessions                                             |
| `APT5_GIT_BASH_PATH`              | chaîne  | Chemin vers l'exécutable Git Bash sur Windows                                     |
| `APT5_CONFIG`                     | chaîne  | Chemin d'accès au fichier de configuration                                        |
| `APT5_TUI_CONFIG`                 | chaîne  | Chemin d'accès au fichier de configuration TUI                                    |
| `APT5_CONFIG_DIR`                 | chaîne  | Chemin d'accès au répertoire de configuration                                     |
| `APT5_CONFIG_CONTENT`             | chaîne  | Contenu de configuration JSON en ligne                                            |
| `APT5_DISABLE_AUTOUPDATE`         | booléen | Désactiver les vérifications automatiques des mises à jour                        |
| `APT5_DISABLE_PRUNE`              | booléen | Désactiver le nettoyage des anciennes données                                     |
| `APT5_DISABLE_TERMINAL_TITLE`     | booléen | Désactiver les mises à jour automatiques des titres du terminal                   |
| `APT5_PERMISSION`                 | chaîne  | Configuration des autorisations JSON intégrées                                    |
| `APT5_DISABLE_DEFAULT_PLUGINS`    | booléen | Désactiver les plugins par défaut                                                 |
| `APT5_DISABLE_LSP_DOWNLOAD`       | booléen | Désactiver les téléchargements automatiques du serveur LSP                        |
| `APT5_ENABLE_EXPERIMENTAL_MODELS` | booléen | Activer les modèles expérimentaux                                                 |
| `APT5_DISABLE_AUTOCOMPACT`        | booléen | Désactiver le compactage automatique du contexte                                  |
| `APT5_DISABLE_CLAUDE_CODE`        | booléen | Désactiver la lecture de `.claude` (prompt + compétences)                         |
| `APT5_DISABLE_CLAUDE_CODE_PROMPT` | booléen | Désactiver la lecture `~/.claude/CLAUDE.md`                                       |
| `APT5_DISABLE_CLAUDE_CODE_SKILLS` | booléen | Désactiver le chargement de `.claude/skills`                                      |
| `APT5_DISABLE_MODELS_FETCH`       | booléen | Désactiver la récupération de modèles à partir de sources distantes               |
| `APT5_FAKE_VCS`                   | chaîne  | Faux fournisseur VCS à des fins de test                                           |
| `APT5_CLIENT`                     | chaîne  | Identifiant du client (par défaut `cli`)                                          |
| `APT5_ENABLE_EXA`                 | booléen | Activer les outils de recherche Web Exa                                           |
| `APT5_SERVER_PASSWORD`            | chaîne  | Activer l'authentification de base pour `serve`/`web`                             |
| `APT5_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `opencode`) |
| `APT5_MODELS_URL`                 | chaîne  | URL personnalisé pour récupérer la configuration des modèles                      |
...
| Variables                                       | Type    | Description                                                     |
| ----------------------------------------------- | ------- | --------------------------------------------------------------- |
| `APT5_EXPERIMENTAL`                         | booléen | Activer les fonctionnalités expérimentales du drapeau global    |
| `APT5_EXPERIMENTAL_ICON_DISCOVERY`          | booléen | Activer la découverte d'icônes                                  |
| `APT5_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | booléen | Désactiver la copie lors de la sélection dans TUI               |
| `APT5_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | numéro  | Délai d'expiration par défaut pour les commandes bash en ms     |
| `APT5_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | numéro  | Nombre maximum de jetons de sortie pour les réponses LLM        |
| `APT5_EXPERIMENTAL_FILEWATCHER`             | booléen | Activer l'observateur de fichiers pour l'ensemble du répertoire |
| `APT5_EXPERIMENTAL_OXFMT`                   | booléen | Activer le formateur oxfmt                                      |
| `APT5_EXPERIMENTAL_LSP_TOOL`                | booléen | Activer l'outil expérimental LSP                                |
| `APT5_EXPERIMENTAL_DISABLE_FILEWATCHER`     | booléen | Désactiver l'observateur de fichiers                            |
| `APT5_EXPERIMENTAL_EXA`                     | booléen | Activer les fonctionnalités Exa expérimentales                  |
| `APT5_EXPERIMENTAL_LSP_TY`                  | booléen | Activer TY LSP pour les fichiers python                         |
| `APT5_EXPERIMENTAL_PLAN_MODE`               | booléen | Activer le mode plan                                            |
| `APT5_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | booléen | Activer les tâches de sous-agents en arrière-plan               |
| `APT5_EXPERIMENTAL_EVENT_SYSTEM`            | booléen | Activer le système d'événements expérimental                    |
| `APT5_EXPERIMENTAL_NATIVE_LLM`              | booléen | Activer le chemin de requête LLM natif                          |
| `APT5_EXPERIMENTAL_PARALLEL`                | booléen | Activer l'exécution parallèle de la recherche web               |
| `APT5_EXPERIMENTAL_SCOUT`                   | booléen | Activer le sous-agent Scout                                     |
| `APT5_EXPERIMENTAL_WORKSPACES`              | booléen | Activer la prise en charge des espaces de travail               |
