...
:::note
Cet outil n'est disponible que lorsque `APT5_EXPERIMENTAL_LSP_TOOL=true` (ou `APT5_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur OpenCode ou lorsque la variable d'environnement `APT5_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).
