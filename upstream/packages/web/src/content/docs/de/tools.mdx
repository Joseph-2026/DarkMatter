...
:::note
Dieses Tool ist nur verfuegbar, wenn `APT5_EXPERIMENTAL_LSP_TOOL=true` (oder `APT5_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der OpenCode-Provider genutzt wird oder die Umgebungsvariable `APT5_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.
