...
Wenn Ihre globale Konfiguration beispielsweise `theme: "darkmatter"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
3. **Benutzerdefinierte Konfiguration** (`APT5_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`opencode.json` im Projekt) – projektspezifische Einstellungen
5. **`.apt5` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`APT5_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.apt5` und `~/.config/opencode` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Geben Sie mithilfe der Umgebungsvariablen `APT5_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `APT5_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.apt5` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.apt5` geladen, sodass es deren Einstellungen **überschreiben** kann.

---

## Schema

Die Konfigurationsdatei verfügt über ein Schema, das in [**`github.com/Joseph-2026/DarkMatter/config.json`**](https://github.com/Joseph-2026/DarkMatter/config.json) definiert ist.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/opencode/agents/` oder `.apt5/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/opencode/commands/` oder `.apt5/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.apt5/plugins/` oder `~/.config/opencode/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
