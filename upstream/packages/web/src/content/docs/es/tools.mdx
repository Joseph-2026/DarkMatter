...
:::note
Esta herramienta solo está disponible cuando `APT5_EXPERIMENTAL_LSP_TOOL=true` (o `APT5_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCode o cuando la variable de entorno `APT5_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).
