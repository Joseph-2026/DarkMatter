---
title: Server
description: Interact with apt-5 server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `apt-5 serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an apt-5 client can use.
...
```bash
apt-5 serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `apt5.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
apt-5 serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `APT5_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `darkmatter`, or set `APT5_SERVER_USERNAME` to override it. This applies to both `apt-5 serve` and `apt-5 web`.

```bash
APT5_SERVER_PASSWORD=your-password apt-5 serve
```
...
When you run `apt-5` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the apt-5 server to interact with apt-5 programmatically.
:::

This architecture lets apt-5 support multiple clients and allows you to interact with apt-5 programmatically.

You can run `apt-5 serve` to start a standalone server. If you have the
apt-5 TUI running, `apt-5 serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the APT-5 [IDE](/docs/ide) plugins.
...
The apt-5 server exposes the following APIs.
