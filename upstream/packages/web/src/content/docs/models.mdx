...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start APT-5.
...
Here are several models that work well with APT-5, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [the curated model gateway](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="apt5.jsonc" {7-12,19-24}
{
  "$schema": "https://github.com/Joseph-2026/DarkMatter/config.json",
  "provider": {
    "openai": {
...
Many models support multiple variants with different configurations. APT-5 ships with built-in default variants for popular providers.

### Built-in variants

APT-5 ships with default variants for many providers:
...
When APT-5 starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the APT-5 config.
