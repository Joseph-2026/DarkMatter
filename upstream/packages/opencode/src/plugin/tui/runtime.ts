...
import { errorData, errorMessage } from "@apt5/tui/util/error"
...
import { hasTheme, upsertTheme } from "@apt5/tui/context/theme"
import { Global } from "@apt5/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@apt5/core/util/flock"
import { Flag } from "@apt5/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@apt5/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@apt5/core/v1/config/plugin"
import { createCommandShim } from "@apt5/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@apt5/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".apt5"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".apt5", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".apt5", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.APT5_PURE ? [] : pluginOrigins
    if (Flag.APT5_PURE && pluginOrigins.length) {
    }
