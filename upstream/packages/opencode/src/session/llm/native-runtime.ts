...
import type { LLMClientShape } from "@apt5/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("darkmatter"))
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @apt5/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @apt5/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
