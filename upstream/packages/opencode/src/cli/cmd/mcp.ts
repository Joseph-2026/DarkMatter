...
import { ConfigMCPV1 } from "@apt5/core/v1/config/mcp"
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in apt5.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .apt5/ subdirectory too)
  const candidates = [
    path.join(baseDir, "apt5.json"),
    path.join(baseDir, "apt5.jsonc"),
  ...
  if (!global) {
    candidates.push(
      path.join(baseDir, ".apt5", "apt5.json"),
      path.join(baseDir, ".apt5", "apt5.jsonc"),
      path.join(baseDir, ".apt5", "opencode.json"),
      path.join(baseDir, ".apt5", "opencode.jsonc"),
    )
  }
