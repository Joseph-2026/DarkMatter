import type { PermissionV1 } from "@apt5/core/v1/permission"
...
import { createApt5Client, type Apt5Client, type ToolPart } from "@apt5/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<Apt5Client["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to APT5_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to APT5_SERVER_USERNAME or 'opencode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: Apt5Client): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: Apt5Client, sessionID: string) {
      ...
      async function current(sdk: Apt5Client): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: Apt5Client) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: Apt5Client) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: Apt5Client) {
        ...
        async function loop(client: Apt5Client, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
