...
import { IdeEvent } from "@apt5/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["APT5_CALLER"] === "vscode" || process.env["APT5_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.apt5"], {
    nothrow: true,
  })
