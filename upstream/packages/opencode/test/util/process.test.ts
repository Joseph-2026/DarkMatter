...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.APT5_TEST ?? "")'), {
      env: {
        APT5_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "APT5_TEST_SHELL"], {
      shell: true,
      env: {
        APT5_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("APT5_TEST_SHELL=ok")
  })
