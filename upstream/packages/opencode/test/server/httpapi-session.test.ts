...
const originalWorkspaces = Flag.APT5_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.APT5_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.APT5_EXPERIMENTAL_WORKSPACES = true
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "darkmatter", "src")
