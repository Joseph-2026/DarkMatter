...
const original = {
  APT5_SERVER_PASSWORD: Flag.APT5_SERVER_PASSWORD,
  APT5_SERVER_USERNAME: Flag.APT5_SERVER_USERNAME,
  envPassword: process.env.APT5_SERVER_PASSWORD,
  envUsername: process.env.APT5_SERVER_USERNAME,
}
const auth = { username: "darkmatter", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.APT5_SERVER_PASSWORD = original.APT5_SERVER_PASSWORD
  Flag.APT5_SERVER_USERNAME = original.APT5_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.APT5_SERVER_PASSWORD
  else process.env.APT5_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.APT5_SERVER_USERNAME
  else process.env.APT5_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.APT5_SERVER_PASSWORD = auth.password
  Flag.APT5_SERVER_USERNAME = auth.username
  process.env.APT5_SERVER_PASSWORD = auth.password
  process.env.APT5_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.APT5_SERVER_PASSWORD = undefined
  Flag.APT5_SERVER_USERNAME = auth.username
  delete process.env.APT5_SERVER_PASSWORD
  process.env.APT5_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.APT5_DISABLE_DEFAULT_PLUGINS
    process.env.APT5_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.APT5_DISABLE_DEFAULT_PLUGINS
      else process.env.APT5_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
