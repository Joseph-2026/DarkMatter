...
import { EventSequenceTable } from "@apt5/core/event/sql"
...
import { WorkspaceTable } from "@apt5/core/control-plane/workspace.sql"
...
const originalEnv = {
  APT5_AUTH_CONTENT: process.env.APT5_AUTH_CONTENT,
  APT5_EXPERIMENTAL_WORKSPACES: process.env.APT5_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.APT5_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.APT5_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.APT5_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.APT5_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.APT5_EXPERIMENTAL_WORKSPACES).toBe("true")
