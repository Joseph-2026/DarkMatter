...
`,
  ...
  const pure = process.env.APT5_PURE
  const meta = process.env.APT5_PLUGIN_META_FILE
  process.env.APT5_PURE = "1"
  process.env.APT5_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.APT5_PURE
    } else {
      process.env.APT5_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.APT5_PLUGIN_META_FILE
    } else {
      process.env.APT5_PLUGIN_META_FILE = meta
    }
  }
})
