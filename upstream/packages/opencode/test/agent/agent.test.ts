...
it.instance("plan agent denies edits except .apt5/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".apt5/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".apt5", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.APT5_TEST_HOME
      process.env.APT5_TEST_HOME = test.directory
