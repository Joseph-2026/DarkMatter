...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "apt5-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["APT5_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { APT5_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("APT5_TEST_SHELL=ok")
      }),
    )
