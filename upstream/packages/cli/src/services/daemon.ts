...
import { ServerAuth } from "@apt5/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createApt5Client>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createApt5Client({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createApt5Client({ baseUrl: connection.url, headers: connection.headers })
    })
