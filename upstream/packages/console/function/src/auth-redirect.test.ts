...
describe("authorization redirect validation", () => {
  test("allows registered OpenCode callbacks", () => {
    expect(isAllowedAuthorizationRedirect("app", "https://github.com/Joseph-2026/DarkMatter/auth/callback")).toBe(true)
    expect(isAllowedAuthorizationRedirect("app", "https://dev.apt5.ai/auth/callback")).toBe(true)
  ...
  test("rejects unregistered clients and external redirects", () => {
    expect(isAllowedAuthorizationRedirect("other", "https://github.com/Joseph-2026/DarkMatter/auth/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://evil.example/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://github.com/Joseph-2026/DarkMatter.evil.example/callback")).toBe(false)
