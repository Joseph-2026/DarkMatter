...
import { PromptInput } from "@apt5/schema/prompt-input"
import { Session } from "@apt5/schema/session"
import { Project } from "@apt5/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@apt5/schema/schema"
...
import { SessionEvent } from "@apt5/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this APT-5 process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this APT-5 process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
