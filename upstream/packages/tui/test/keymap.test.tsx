...
import { getApt5ModeStack, APT5_BASE_MODE, Apt5KeymapProvider, registerApt5Keymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerApt5Keymap(keymap, renderer, config)
...
test("mode-less bindings stay active when opencode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getApt5ModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getApt5ModeStack(keymap).push("autocomplete")
