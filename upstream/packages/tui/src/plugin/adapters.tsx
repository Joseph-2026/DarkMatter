import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@apt5/plugin/tui"
...
import type { useApt5Keymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useApt5Keymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getApt5ModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getApt5ModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
