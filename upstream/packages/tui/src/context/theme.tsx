...
const themeSource: ThemeSource = {
  async discover() {
    ...
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      directories.push(path.join(current, ".apt5"))
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const raw = config.theme ?? kv.get("theme", "apt5")
        const active = raw === "opencode" ? "apt5" : raw
        draft.active = typeof active === "string" ? active : "apt5"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "apt5")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.apt5, store.mode)
    })
