import type { TuiPluginApi } from "@apt5/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("apt5.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public github.com/Joseph-2026/DarkMatter link",
  ...
  "Create {highlight}apt5.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/darkmatter/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.apt5/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.apt5/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.apt5/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.apt5/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent APT-5 from reading sensitive files",
  "Use {highlight}apt-5 run{/highlight} for non-interactive scripting",
  "Use {highlight}apt-5 --continue{/highlight} to resume the last session",
  "Use {highlight}apt-5 run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}apt-5 serve{/highlight} for headless API access to APT-5",
  "Use {highlight}apt-5 run --attach{/highlight} to connect to a running server",
  ...
  "Run {highlight}apt-5 debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}apt-5 --help{/highlight} to see all commands and options",
  "Use {highlight}/connect{/highlight} to add model providers with free and curated models",
