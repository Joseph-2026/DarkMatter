...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `apt-5 models` to list available models",
      "Or check your config (apt5.json) provider/model names",
    ].join("\n")
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`apt-5 auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, APT-5 does not support MCP authentication yet.`
  }
  return undefined
}
