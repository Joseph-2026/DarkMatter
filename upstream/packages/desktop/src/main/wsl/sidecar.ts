...
export async function spawnWslSidecar(
  ...
  if (!opencode) throw new Error(nativeT("desktop.wsl.error.apt5NotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "darkmatter"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export APT5_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export APT5_CLIENT=desktop",
    `export APT5_SERVER_USERNAME=${shellEscape(username)}`,
    `export APT5_SERVER_PASSWORD=${shellEscape(password)}`,
