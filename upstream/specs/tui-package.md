...
- Do not replace private `packages/opencode` imports with `@apt5/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@apt5/tui`.
...
Status: Completed. The private `@apt5/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@apt5/tui`.
...
- The package has no dependency on `opencode`, `@apt5/cli`, or
  `@apt5/core`.
...
primitives now live in `@apt5/tui`. The legacy host consumes explicit
...
and build inputs through `@apt5/tui/runtime`. Movable app, component,
...
`@apt5/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@apt5/tui`. Configured references resolve through the new
...
`@apt5/tui`. The legacy host injects a narrow plugin host that retains
...
public `@apt5/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@apt5/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@apt5/core`, or either
  executable package.
...
rg '@apt5/core|packages/opencode|packages/cli' packages/tui
