...
| `sdks/<lang>/` | Generated output (python, go, rust, java, csharp, ruby, php, swift, kotlin, javascript, typescript-axios, typescript-fetch, dart, elixir, scala, perl, r, lua, powershell, bash) | **no** (see below) |
...
- `COMPILE_OK` — generated code compiles with a local toolchain (python, go, ruby, php).
- `GEN_ONLY` — generation succeeded; no local toolchain (rust, swift, kotlin, …). CI asserts presence + markers; consumers compile on their side.
